...
static ssize_t my_write(struct file *f, const char __user *buf, size_t len, loff_t *off)
{
    char kbuf[100];
    if (len > 99)
        len = 99;  // Limitar el tamaño del buffer para evitar desbordamiento
    if (copy_from_user(kbuf, buf, len))
        return -EFAULT;
    kbuf[len] = '\0';  // Asegurarse de que el buffer es un string válido
...
static int __init drv3_init(void) /* Constructor */
{
    ...
    // En versiones del kernel 2.4 y anteriores, la creación automática de "device files" 
